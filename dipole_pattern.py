...
def main():
    ...
    # Print gain at az=0 for el=10 to 80 deg in 10 deg steps, for multiple heights
    ...
    for el in range(10, 90, 10):
        ...
        for h_m in heights_m:
            result = run_pymininec(
                ...
                ground_opts=ground_opts,
                excitation_pulse="10,1",
                pattern_opts={"theta": f"{el},0,1", "phi": "0,0,1"},
                option="far-field",
            )
            pattern = result['pattern']
            gain = None
            for p in pattern:
                if abs(p['el']-el)<1e-3 and abs(p['az']-0)<1e-3:
                    gain = p['gain']
                    break
            if gain is not None:
                row += f" {gain:7.3f}"
