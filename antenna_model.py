...
def build_two_element_yagi_model(
    ...
    Both elements centered on z=0, oriented along y-axis, spaced along x-axis.
    Driven element is center-fed.
    Returns an AntennaModel with one feedpoint on the driven element.
    """
    # Convert wavelength (m)
    c = 299792458.0
    wavelength_m = c / (freq_mhz * 1e6)
    spacing_m = spacing_frac * wavelength_m
    # Half-lengths
    half_driven = resonant_dipole_length(freq_mhz) / 2.0
    # Reflector length tuned by detune_frac via frequency scaling
    passive_length = resonant_dipole_length(freq_mhz / (1.0 + detune_frac))
    half_passive = passive_length / 2.0
    # Build model
    model = AntennaModel()
    # Driven element at origin along y-axis
    el1 = AntennaElement(
    ...
    center_seg = (segments + 1) // 2
    model.add_feedpoint(element_index=0, segment=center_seg)
    # Passive reflector element offset along x-axis by spacing_m
    el2 = AntennaElement(
